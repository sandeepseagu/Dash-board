...
$query=sprintf("SELECT SUM(price) AS total_sales from transactions WHERE DATE(time_trans)>='{$limit_1}' AND DATE(time_trans)<='{$limit_2}' and type_machine!='Terminal'".$store_query."");
...
$query2=sprintf("SELECT SUM(price) AS total_sales from transactions WHERE DATE(time_trans)>='{$limit_3}' AND DATE(time_trans)<='{$limit_4}' and type_machine!='Terminal'".$store_query."");
